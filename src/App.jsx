    ...
    try {
      ...
      if (uploadedFile.state === "PROCESSING") {
        ...
        await waitForActiveFile(uploadedFile.name, apiKey);
            ...
            setStage(`Analyzing documents with ${selectedModel} (Pass 1)...`);

            const pass1Res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${selectedModel}:generateContent?key=${apiKey}`, {
                method: "POST",
            ...
            if (!pass1Res.ok) {
                let errStr = `API error: ${pass1Res.status}`;
                try {
                    const errData = await pass1Res.json();
            ...
            if (!data1.candidates || data1.candidates.length === 0) {
                throw new Error("No response generated by the AI model during Pass 1.");
            }

            const rawText1 = data1.candidates[0].content.parts[0].text;
            const jsonString1 = rawText1.replace(/```json/g, "").replace(/```/g, "").trim();

            setStage(`Verifying accuracy against original PDF (Pass 2)...`);

            const VERIFY_PROMPT = `You are an Expert Medical Records Verifier. Below is a JSON list of medical records extracted during a first pass from the attached PDF. 
Your job is to strictly verify this list against the original PDF to ensure 100% accuracy.
1. Find any documents that were MISSED (like small pharmacy receipts or procedure addendums) and ADD them.
2. REMOVE any duplicates.
3. Look at all MRI/X-Ray/CT reports: verify that the Doctor Name in the list matches the EXACT Radiologist Electronic Signature at the BOTTOM of the page. If the first pass wrongly used the 'Referring Physician' from the top of the page, CORRECT it.
4. Ensure all formatting heavily matches the original system format (e.g. grouped follow-up visits, "Provider Name, MD", etc).
5. Output the final, perfect JSON object in the exact same format.

=== FIRST PASS JSON ===
${jsonString1}`;

            const pass2Res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${selectedModel}:generateContent?key=${apiKey}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    contents: [
            ...
            if (!pass2Res.ok) {
                let errStr = `API error Pass 2: ${pass2Res.status}`;
                try {
                    const errData = await pass2Res.json();
                    errStr = errData.error?.message || errStr;
                } catch (e) { }
                throw new Error(errStr);
            }

            setStage("Parsing verified results...");
            const data2 = await pass2Res.json();
            if (!data2.candidates || data2.candidates.length === 0) {
                throw new Error("No response generated by the AI model during Pass 2.");
            }

            const rawText2 = data2.candidates[0].content.parts[0].text;
            const jsonString2 = rawText2.replace(/```json/g, "").replace(/```/g, "").trim();

            let parsed;
            try {
                parsed = JSON.parse(jsonString2);
            } catch (parseErr) {
                console.error("Failed to parse JSON:", jsonString2);
